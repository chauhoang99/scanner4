            ...
            **s,
            ...
            "Time Filter": (
                ...
                if time_filter_enabled and granularity == "D"
                      ...
                      if time_filter_enabled and granularity in ("H4", "H8") else "All")
            ),
        })
    return out
...
def main():
    ...
    with st.sidebar:
        ...
        time_filter_enabled = st.checkbox(
            ...
            value=False,
            disabled=granularity not in ("D", "H4", "H8"),
            help=(
                "Daily: use only historical setups whose B candle is on the same weekday "
                "as the current B candle. H4/H8: use only historical setups whose B candle "
                "starts in the same timeframe slot as the current B candle. Other timeframes are unchanged."
            ),
        )
